...
function parseScriptFilesArg(scripts) {
  return (typeof scripts === 'string'? scripts.split(',') : []).map(script => path.resolve(process.cwd(), script));
}

function parseAssetFilesArg(assets) {
  return (typeof assets === 'string'? assets.split(',') : []).map(asset => path.resolve(process.cwd(), asset));
}
...
function parseOptions(args) {
  ...
  options.scriptFilePaths = parseScriptFilesArg(args.scripts);

  options.assetFilePaths = parseAssetFilesArg(args.assets);
